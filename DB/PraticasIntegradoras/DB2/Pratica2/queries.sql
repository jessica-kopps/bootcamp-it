...
SELECT employees.first_name, employees.job, departments.location
FROM employees
INNER JOIN departments
ON employees.department_id = departments.id;
-- 2. Visualize departamentos com mais de cinco funcionários.
SELECT departments.id, departments.name, departments.location, 
COUNT(*) AS employees_number FROM departments
...
SELECT employees.first_name, employees.salary, departments.name
FROM employees
INNER JOIN departments
ON employees.department_id = departments.id
...
-- ordenados por nome.
SELECT employees.first_name, employees.last_name, employees.job, employees.entry_date, departments.name
FROM employees
...
SELECT employees.id, employees.first_name, employees.last_name, employees.job, employees.salary 
FROM employees
INNER JOIN departments ON employees.department_id = departments.id
